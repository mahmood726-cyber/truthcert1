...
// Reference: Voracek et al. (2019), Götz (2024)
// ============================================================================
...
function permutationTestI2(yi, vi, nPerm = 1000) {
  ...
  // P-value (proportion of permuted I² >= observed)
...
function limitMetaAnalysis(yi, vi) {
  ...
  const beta1 = sumWXY / sumWX2;
  const beta0 = meanY - beta1 * meanPrec; // Intercept = limit as precision → ∞
...
function valueOfInformation(theta, se, costPerPatient, nPotentialPatients) {
  ...
  // EVPI = E[max(0, θ)] - max(0, E[θ])

  const mu = theta;
  const sigma = se;

  // E[max(0, θ)] where θ ~ N(mu, sigma²)
  // = mu × Φ(mu/sigma) + sigma × φ(mu/sigma)
...
function albatrossPlotData(yi, vi, names = null) {
  ...
  // SE(logOR) ≈ sqrt(1/a + 1/b + 1/c + 1/d) ≈ sqrt(4/n) for balanced
 ...
 * Equivalent to metafor's rma.glmm() - handles zero cells without continuity correction
 * Uses exact binomial-normal model with numerical integration
 */
function glmmMetaAnalysis(ai, bi, ci, di, options = {}) {
  const k = ai.length;
  const measure = options.measure || 'OR';
  const method = options.method || 'ML';
  const nQuad = options.nQuad || 13; // Gauss-Hermite quadrature points
  ...
  // Handle zero cells - exact method doesn't need continuity correction
  const hasZeroCells = ai.some((a, i) => a === 0 || bi[i] === 0 || ci[i] === 0 || di[i] === 0);

  // Gauss-Hermite quadrature nodes and weights
  const ghNodes = gaussHermiteNodes(nQuad);

  // Log-likelihood function for binomial-normal model
  function logLikelihood(mu, tau) {
    let ll = 0;
    for (let i = 0; i < k; i++) {
      // Numerical integration over random effect
      let integral = 0;
  ...
  // Grid search optimization for ML estimates
...
function gaussHermiteNodes(n) {
  // Pre-computed for common values
  const tables = {
    7: {
    ...
    21: {
      ...
      weights: new Array(21).fill(1/21) // Simplified
    }
  };
 ...
 * Alternative to GLMM for handling overdispersion
 */
...
/**
 * Profile Likelihood for tau² and other parameters
...
function likelihoodRatioTest(yi, vi) {
  ...
  // P-value (mixture of chi-squared, since tau² >= 0)
...
function saveProject(filename = null) {
  // Get data from AppState.data or extract from results if available
