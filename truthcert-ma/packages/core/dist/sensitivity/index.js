...
exports.Sensitivity = exports.fragilityIndex = exports.goshData = exports.baujatData = exports.influence = exports.cumulative = exports.leaveOneOut = void 0;
 ...
 */
function leaveOneOut(effects, options = {}) {
    const results = [];
    for (let i = 0; i < effects.length; i++) {
        const subset = effects.filter((_, j) => j !== i);
        const pooled = (0, pooling_1.randomEffects)(subset, options);
        const Q = (0, pooling_1.calculateQ)(subset);
        results.push({
            omitted: effects[i].study || effects[i].id || `Study ${i + 1}`,
            theta: pooled.theta,
            se: pooled.se,
            ci: pooled.ci,
            I2: (0, pooling_1.calculateI2)(Q.value, Q.df)
        });
    }
    return results;
}
 ...
 */
function cumulative(effects, options = {}) {
    const results = [];
    for (let i = 0; i < effects.length; i++) {
 ...
 */
function influence(effects, options = {}) {
    const k = effects.length;
    const yi = effects.map(e => e.yi);
    const vi = effects.map(e => e.vi);
    // Full model
    const fullPooled = (0, pooling_1.randomEffects)(effects, options);
    ...
    const sigmaResid = Math.sqrt(residuals.reduce((s, r, i) => s + wi[i] * r * r, 0) / (k - 1));
    const results = [];
    for (let i = 0; i < k; i++) {
        ...
        const subset = effects.filter((_, j) => j !== i);
        const subPooled = (0, pooling_1.randomEffects)(subset, options);
        ...
        const influential = Math.abs(rstudent) > 2 ||
 ...
 */
function baujatData(effects, options = {}) {
    const yi = effects.map(e => e.yi);
    const vi = effects.map(e => e.vi);
    const fullPooled = (0, pooling_1.randomEffects)(effects, options);
    const tau2 = fullPooled.tau2 || 0;
 ...
 */
function goshData(effects, options = {}, maxCombinations = 10000) {
    const k = effects.length;
 ...
 */
function fragilityIndex(pooledP, ci, k) {
...
exports.fragilityIndex = fragilityIndex;
/**
