...
async function validateCommand(file, options) {
    ...
    if (data.length > 0) {
        ...
        const hasBinary = ('events1' in firstRow && 'total1' in firstRow && 'events2' in firstRow && 'total2' in firstRow) ||
            ('ai' in firstRow && 'bi' in firstRow && 'ci' in firstRow && 'di' in firstRow);
        const hasContinuous = ('mean1' in firstRow && 'sd1' in firstRow && 'n1' in firstRow && 'mean2' in firstRow && 'sd2' in firstRow && 'n2' in firstRow) ||
...
function validateBinaryData(data, errors, warnings, strict) {
    data.forEach((row, i) => {
        ...
        const fields = { events1, total1, events2, total2 };
...
function validateContinuousData(data, errors, warnings, strict) {
    data.forEach((row, i) => {
        ...
        const fields = { mean1, sd1, n1, mean2, sd2, n2 };
        Object.entries(fields).forEach(([field, value]) => {
            if (typeof value !== 'number' || isNaN(value)) {
                errors.push(`Row ${rowNum}: Invalid or missing '${field}'`);
            }
