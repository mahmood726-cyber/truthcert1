...
exports.Bias = exports.analyzeBias = exports.funnelPlotData = exports.trimFill = exports.beggTest = exports.eggerTest = void 0;
...
function eggerTest(effects) {
    ...
    // P-value (two-tailed)
    const df = n - 2;
    const p = 2 * (1 - (0, statistics_1.pt)(Math.abs(t), df));
    // Interpretation
    let interpretation;
    if (p < 0.01) {
...
function beggTest(effects) {
    ...
    const result = (0, statistics_1.kendallTau)(yi, vi);
    return {
        tau: result.tau,
        z: result.z,
        p: result.p
 ...
 */
function trimFill(effects, pooled, side = 'auto', maxIter = 100) {
    const k = effects.length;
    const yi = effects.map(e => e.yi);
    const vi = effects.map(e => e.vi);
    ...
    const p = (0, statistics_1.pFromZ)(z);
    const adjustedCI = {
 ...
 */
function funnelPlotData(effects, pooledTheta) {
    ...
    const pseudoCI = {
        lower: (se) => pooledTheta - 1.96 * se,
        upper: (se) => pooledTheta + 1.96 * se
    };
    return {
        studies,
...
exports.funnelPlotData = funnelPlotData;
/**
 ...
 */
function analyzeBias(effects, pooled) {
